...
//        }
		...
		private void AddTab (string text, int index)
		{
			...
			var tabView = new TabView(this.Context);
		...
		public class TabView : LinearLayout
		{
			...
			{
				View.Inflate(this.Context, Resource.Layout.vpi__tab, this);
			}
			...
			public void Init (TabPageIndicator parent, string text, int index)
			{
                ...
                RelativeLayout tabView = FindViewById<RelativeLayout>(Resource.Id.tabLayout);
                tabView.Background.Colorize(DataManager.Get<ISettingsManager>().Settings.ButtonColor);
