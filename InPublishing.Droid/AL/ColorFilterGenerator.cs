    ...
    public class ColorFilterGenerator
    {
      ...
      9.6, 9.8, 10.0
        ...
        private static float[] getHsv(int color)
        {
            ...
            Color.RGBToHSV(col.R, col.G, col.B, hsv);
            return hsv;
        }
        ...
        private static Bitmap drawableToBitmap(Drawable drawable)
        {
            if (drawable is BitmapDrawable) {
                ...
                Bitmap bmp = Bitmap.CreateBitmap(pictureDrawable.IntrinsicWidth,
                                                    pictureDrawable.IntrinsicHeight, Bitmap.Config.Argb8888);
                Canvas canv = new Canvas(bmp);
                canv.DrawPicture(pictureDrawable.Picture);
                return bmp;
            }
        ...
        private static int[] getAverageColorRGB(Bitmap bitmap)
        {
            ...
            for (int i = 0; i < size; i++)
            {
                ...
                r += Color.GetRedComponent(pixelColor);
                g += Color.GetGreenComponent(pixelColor);
                b += Color.GetBlueComponent(pixelColor);
            }
            r /= size;
            g /= size;
            b /= size;
        ...
        public class From
        {
            ...
            public From(Bitmap bitmap)
            {
                oldColor = getAverageColor(bitmap);
            }

            public From(int oldColor)
            {
